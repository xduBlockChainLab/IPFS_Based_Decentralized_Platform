...
package com.example.paas_decentralized_webplatform.util;
...
public class BlockUtil {
    ...
    public static Block queryBlockByNumber(SdkClient sdkClient, long blockNumber) throws Exception {
        try {
            System.out.println("[info] enter queryBlockByNumber");
            QueryRawMessage queryRawMessage = sdkClient.getQueryRawMessage();
            WienerChainNode wienerChainNode = sdkClient.getWienerChainNode(App.queryNode);
            RawMessage rawMessage = queryRawMessage.buildBlockRawMessage(App.chainID, blockNumber);
    ...
    // 查询当前最新的区块
    public static Block queryLastBlock(SdkClient sdkClient) throws Exception {
        System.out.println("[info] enter queryLastBlock");
        long lastBlockNumber = queryLastBlockNumber(sdkClient);
        return queryBlockByNumber(sdkClient, lastBlockNumber);
    }

    // 通过交易 ID 查询区块
    public static Block queryBlockByTxID(SdkClient sdkClient, String txID) throws Exception {
        System.out.println("[info] enter queryBlockByTxID");
        try {
            byte[] txHash = Hex.decode(txID);
            QueryRawMessage queryRawMessage = sdkClient.getQueryRawMessage();
            WienerChainNode wienerChainNode = sdkClient.getWienerChainNode(App.queryNode);
            RawMessage rawMessage = queryRawMessage.buildTxRawMessage(App.chainID, txHash);
    ...
    // 查询当前最新区块的区块号
    public static long queryLastBlockNumber(SdkClient sdkClient) throws Exception {
        QueryRawMessage queryRawMessage = sdkClient.getQueryRawMessage();
        WienerChainNode wienerChainNode = sdkClient.getWienerChainNode(App.queryNode);
        RawMessage rawMessage = queryRawMessage.buildLatestChainStateRawMessage(App.chainID);
        ...
        System.out.println("[error] queryLastBlockNumber failed, error is: " + response.getStatus() + ": "
    ...
    private static Block parseToBlock(ListenableFuture<RawMessage> future) throws Exception {
        ...
        System.out.println("[error] parse to block failed, error is: " + response.getStatus() + ": "
            + response.getStatusInfo());
        return null;
    }
}
