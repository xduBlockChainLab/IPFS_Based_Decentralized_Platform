...
public class TxUtil {
    ...
    // 通过交易 ID 查询交易
    public static Tx queryTxByTxID(SdkClient sdkClient, String txID) throws Exception {
        System.out.println("[info] enter queryTxByTxID");
        try {
            ...
            System.out.println("[error] parse to transaction failed, error is: " + response.getStatus() + ": "
    ...
    public static String queryTxResultByTxID(SdkClient sdkClient, String txID) throws Exception {
        System.out.println("[info] enter queryTxResultByTxID");
        try {
            byte[] txHash = Hex.decode(txID);
            QueryRawMessage queryRawMessage = sdkClient.getQueryRawMessage();
            WienerChainNode wienerChainNode = sdkClient.getWienerChainNode(App.queryNode);
            ...
            System.out.println("[error] queryTxResultByTxID failed, error is: " + response.getStatus() + ": "
                + response.getStatusInfo());
        } catch (DecoderException e) {
            System.out.println("[error] please input a txID with correct format");
            return null;
    ...
    public static List<String> getEndorsersOrg(Tx tx) throws CryptoException {
        ...
        for (Approval approval : tx.getFull().getApprovalsList()) {
    ...
    private static LdapName getLdapName(byte[] identity) throws CryptoException {
        try {
            ...
            X509Certificate certificate = (X509Certificate) CertificateFactory
                .getInstance(TYPE, PROVIDER_SHORT_NAME).generateCertificate(byteArrayInputStream);
    ...
    private static CommonTxData getCommonTxData(Tx tx) throws InvalidProtocolBufferException {
        ...
        try {
            ...
            System.out.println("[warn] Reading a block with no tx keyValue, "
